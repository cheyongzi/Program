...
+ (void)show {
    [[SVProgressHUD sharedView] showWithStatus:nil maskType:SVProgressHUDMaskTypeGradient networkIndicator:NO];
}
...
+ (void)overlayWindowFrome:(NSString*)status
{
    [[SVProgressHUD sharedView] showWithStatus:status maskType:SVProgressHUDMaskTypeGradient networkIndicator:NO Frome:CGRectMake(0, 0, 0, 0)];
}
...
- (void)showWithStatus:(NSString*)string maskType:(SVProgressHUDMaskType)hudMaskType networkIndicator:(BOOL)show {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if(self.maskType != SVProgressHUDMaskTypeNone) {
            ...
            self.overlayWindow.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y+64, self.frame.size.width, self.frame.size.height);
            self.overlayWindow.backgroundColor = [[UIColor alloc]initWithRed:0 green:0 blue:0 alpha:0];
        } else {
...
- (void)dismissWithStatus:(NSString *)string error:(BOOL)error afterDelay:(NSTimeInterval)seconds {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        self.overlayWindow.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, self.frame.size.width, self.frame.size.height);
    });
}
...
- (void)showWithStatus:(NSString*)string maskType:(SVProgressHUDMaskType)hudMaskType networkIndicator:(BOOL)show Frome:(CGRect)frome{
    dispatch_async(dispatch_get_main_queue(), ^{
        if(!self.superview)
            [self.overlayWindow addSubview:self];
